...
const deployCustomKIP7TokenAsFeePayer = async (rlpEncodingString) => {
  try {
    ...
    const deployTransaction = caver.transaction.feeDelegatedSmartContractDeploy.create(rlpEncodingString);
    ...
    const receipt = await caver.rpc.klay.sendRawTransaction(signedTransaction.getRawTransaction());
